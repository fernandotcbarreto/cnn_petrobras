...
class DataGenerator:
    def __init__(self, company_code, data_path='./stock_history', output_path='./outputs', strategy_type='original',
                 update=False, logger: Logger = None, dfinput=pd.DataFrame({'A' : []})):
...
#        self.log("creating label with {}_{}_ma".format(short, long))
    ...
    def create_features(self):
        if not os.path.exists(os.path.join(self.output_path, "df_" + self.company_code+".csv")) or self.update:
            df=self.dfinput
            if df.empty:
              df = pd.read_csv(os.path.join(self.data_path,self.company_code+'.csv'), engine='python')
#            print(df.head())
            print('Processing .csv file '+os.path.join(self.data_path,self.company_code+'.csv'))
